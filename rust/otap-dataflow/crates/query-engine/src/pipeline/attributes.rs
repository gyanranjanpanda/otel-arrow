...
mod test {
    ...
    async fn test_insert_attributes<P: Parser>() {
        ...
        // Test inserting into resource attributes
        let result = exec_logs_pipeline::<P>(
            "logs | extend resource.attributes[\"new_res_attr\"] = \"res_val\"",
            generate_logs_test_data(),
        )
        .await;

        assert_eq!(
            result.resource_logs[0].resource.as_ref().unwrap().attributes,
            &[
                KeyValue::new("xr1", AnyValue::new_string("a")),
                KeyValue::new("xr2", AnyValue::new_string("a")),
                KeyValue::new("new_res_attr", AnyValue::new_string("res_val")),
            ]
        );
    }
